...
describe("generateId", () => {
	test("generates team IDs with date component", () => {
		const id = generateId("team");
		assert.match(id, /^team-\d{8}-\d{3}$/);
	});
...
describe("TeamStore — directory helpers", () => {
	...
	test("ensureTeamDirs creates directory tree", async () => {
		...
		await assert.doesNotReject(() =>
			access(
				join(dir, ".pi", "teams", "team-001", "teammates", "backend", "outputs"),
			),
		);
		await rm(dir, { recursive: true, force: true });
	});
...
describe("TeamStore — signal operations", () => {
	...
	test("loadSignalsSince filters signals by timestamp", async () => {
		...
		await store.appendSignal(team.id, makeSignal({ id: "sig-001", timestamp: ts1 }));
		await store.appendSignal(team.id, makeSignal({ id: "sig-002", timestamp: ts2 }));
		await store.appendSignal(team.id, makeSignal({ id: "sig-003", timestamp: ts3 }));
...
describe("TeamStore — mailbox operations", () => {
	...
	test("loadMessagesFor returns direct and broadcast messages", async () => {
		...
		await store.appendMessage(team.id, makeMessage({ id: "msg-001", to: "frontend", message: "direct" }));
		await store.appendMessage(team.id, makeMessage({ id: "msg-002", to: "all", message: "broadcast" }));
		await store.appendMessage(team.id, makeMessage({ id: "msg-003", to: "backend", message: "other" }));
	...
	test("loadMessagesFor returns leader messages and broadcasts when recipient is leader", async () => {
		...
		await store.appendMessage(team.id, makeMessage({ id: "msg-001", to: "leader", message: "to leader" }));
		await store.appendMessage(team.id, makeMessage({ id: "msg-002", to: "all", message: "broadcast" }));
		await store.appendMessage(team.id, makeMessage({ id: "msg-003", to: "backend", message: "other" }));
...
describe("TeamStore — approval operations", () => {
	test("saveApprovals then loadApprovals returns same list", async () => {
		...
		const approvals = [makeApproval({ id: "apr-001" }), makeApproval({ id: "apr-002" })];
