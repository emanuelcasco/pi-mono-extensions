...
import { TEAMMATE_ROLE_PROMPTS } from "../core/types.js";
...
async function writePromptToTempFile(prefix: string, prompt: string): Promise<{ dir: string; filePath: string }> {
	const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), `pi-teams-${prefix}-`));
...
function buildTaskPrompt(task: TaskRecord): string {
	...
	if (task.artifacts.length > 0) lines.push(`Artifacts to produce or update: ${task.artifacts.join(", ")}`);
	if (task.blockers.length > 0) lines.push(`Known blockers: ${task.blockers.join("; ")}`);
	return lines.join("\n\n");
}
...
function summarizeCompletionOutput(output: string, fallback: string): string {
	const lines = output
		...
		.filter(
			(line) =>
				!/^#+\s*/.test(line) &&
				!/^(what was accomplished|files created or modified|issues or open questions|handoff notes(?: for other teammates)?|handoffs?)\s*:?$/i.test(line),
		);
...
function parseExplicitHandoffs(output: string, teammates: string[], sender: string): ParsedHandoff[] {
	...
	for (const rawLine of lines) {
		...
		if (/^(?:#{1,6}\s*)?handoff notes(?: for other teammates)?\s*:?$/i.test(line) || /^(?:#{1,6}\s*)?handoffs\s*:?$/i.test(line) || /^4\.\s*handoff notes(?: for other teammates)?\s*:?$/i.test(line)) {
		...
		let match = normalized.match(/^to\s*:\s*([a-z0-9_-]+)\s*\|\s*message\s*:\s*(.+)$/i);
		if (match) {
...
export class LeaderRuntime {
	...
	async launchLeader(teamId: string): Promise<void> {
		...
		await this.teamManager.updateTeam(teamId, {
			...
			summary: team.summary ?? `Leader started for objective: ${team.objective}`,
		});
		...
		try {
			...
			await writeFile(path.join(promptDir, "prompt.md"), this.buildLeaderPrompt(team), "utf8");
		} catch {
			// best effort only
		}
	...
	async spawnTeammate(
		...
		const effectiveCwd = cwd ?? task.worktree ?? team.repoRoots[0] ?? process.cwd();
		const prompt = this.buildTeammatePrompt(team.name, role, taskDescription, context, effectiveCwd);
		const tempPrompt = await writePromptToTempFile(`teammate-${safeKebab(`${teamId}-${role}`)}`, prompt);

		const args = ["--mode", "json", "-p", "--no-session", "--append-system-prompt", tempPrompt.filePath, `Task: ${taskDescription}`];
		...
		await this.taskManager.updateTask(teamId, taskId, { status: "in_progress", owner: role, blockers: [] });
		await this.signalManager.emit(teamId, {
			source: role,
			type: "task_started",
			severity: "info",
		...
		proc.on("close", (code) => {
			void (async () => {
				...
				if (code === 0) {
					const finalState: TeammateProcess = {
						...processState,
						state: "completed",
						completedAt,
						output,
					};
					await this.store.saveTeammateProcess(teamId, finalState);
					...
					await this.taskManager.updateTask(teamId, taskId, {
						status: "done",
						artifacts: output.trim() ? [...task.artifacts, `teammates/${role}/outputs/${outputFile}`] : task.artifacts,
					});
					await this.signalManager.emit(teamId, {
						source: role,
						type: "task_completed",
		...
		proc.on("error", (err) => {
			void (async () => {
				...
				const message = err.message || `Failed to spawn teammate ${role}`;
				await this.store.saveTeammateProcess(teamId, {
					...processState,
					state: "failed",
					completedAt,
	...
	private async ensureBootstrapTasks(teamId: string): Promise<void> {
		...
		const researchOwner = team.teammates.find((role) => role === "researcher" || role === "docs");
		const plannerOwner = team.teammates.find((role) => role === "planner") ?? team.teammates[0];
		const implementationOwners = team.teammates.filter((role) => ["backend", "frontend", "tester", "docs"].includes(role));
		...
		const planTask = created.find((task) => task.title.startsWith("Create implementation plan"));
		const implementationDepends = planTask ? [planTask.id] : created.map((task) => task.id);
	...
	private async runLeaderCycle(teamId: string): Promise<void> {
		const team = await this.store.loadTeam(teamId);
		if (!team) return;
		if (team.status === "cancelled" || team.status === "completed" || team.status === "failed") {
			return;
		}
		...
		if (refreshedTasks.length > 0 && refreshedTasks.every((task) => task.status === "done" || task.status === "cancelled")) {
			await this.teamManager.updateTeam(teamId, {
				status: "completed",
			...
			await this.store.saveLeaderProcess(teamId, {
				teamId,
				state: "completed",
				startedAt: (await this.store.loadLeaderProcess(teamId))?.startedAt ?? new Date().toISOString(),
				completedAt: new Date().toISOString(),
			});
			await this.signalManager.emit(teamId, {
				source: "leader",
		...
		const readyTasks = refreshedTasks
			...
			.sort((a, b) => (a.priority === b.priority ? a.createdAt.localeCompare(b.createdAt) : a.priority === "high" ? -1 : b.priority === "high" ? 1 : 0));
		...
		for (const task of readyTasks) {
			...
			try {
				await this.spawnTeammate(teamId, task.owner, task.id, buildTaskPrompt(task), context, task.worktree ?? team.repoRoots[0]);
			} catch (err) {
		...
		const summaryText = `${summary.progress.done}/${summary.progress.total} tasks done` +
			(summary.blockers.length > 0 ? `, ${summary.blockers.length} blocker(s)` : "") +
			(summary.approvalsPending.length > 0 ? `, ${summary.approvalsPending.length} approval(s) pending` : "");
	...
	private determinePhase(tasks: TaskRecord[], currentPhase: LeaderPhase): LeaderPhase {
		const hasResearch = tasks.some((task) => task.title.startsWith("Research "));
		const researchPending = tasks.some((task) => task.title.startsWith("Research ") && task.status !== "done" && task.status !== "cancelled");
		const synthesisPending = tasks.some((task) => task.title.startsWith("Create implementation plan") && task.status !== "done" && task.status !== "cancelled");
		const implementationPending = tasks.some((task) => task.title.startsWith("Implement ") && task.status !== "done" && task.status !== "cancelled");
		const verificationPending = tasks.some((task) => task.title.startsWith("Review completed work") && task.status !== "done" && task.status !== "cancelled");
	...
	private async buildTaskContext(teamId: string, task: TaskRecord): Promise<string> {
		const [summary, signals, taskMailbox, directMailbox, team] = await Promise.all([
			...
			task.owner ? this.mailboxManager.getMessagesFor(teamId, task.owner) : Promise.resolve([]),
			this.store.loadTeam(teamId),
		]);

		const recentSignals = signals.slice(-8).map((signal) => `- [${signal.type}] ${signal.source}: ${signal.message}`);
		const mailbox = [...new Map([...directMailbox, ...taskMailbox].map((message) => [message.id, message])).values()]
		...
		return [
			...
			task.dependsOn.length > 0 ? `Dependencies: ${task.dependsOn.join(", ")}` : "Dependencies: none",
			recentSignals.length > 0 ? `Recent signals:\n${recentSignals.join("\n")}` : "Recent signals: none",
			messages.length > 0 ? `Mailbox:\n${messages.join("\n")}` : "Mailbox: none",
		].join("\n\n");
	}
	...
	private buildLeaderPrompt(team: TeamRecord): string {
		return [
			...
			"",
			"## Your Teammates",
			team.teammates.length > 0 ? team.teammates.map((role) => `- ${role}`).join("\n") : "- none",
			"",
			"## Phases",
	...
	private async automateTeammateHandoffs(
		...
		const explicitHandoffs = parseExplicitHandoffs(output, team.teammates, fromRole);
		const explicitByRecipient = new Map(explicitHandoffs.map((handoff) => [handoff.to, handoff.message]));
		const completionSummary = summarizeCompletionOutput(output, `Completed ${completedTask.title}.`);
		...
		for (const recipient of recipients) {
			...
			const recipientTask = downstreamTasks.find((task) => task.owner === recipient)
				?? allTasks.find(
					(task) =>
			...
			const message = explicitByRecipient.get(recipient) ?? `${autoContext} ${completionSummary}`;

			const mailboxMessage = await this.mailboxManager.send(teamId, {
				from: fromRole,
				to: recipient,
				taskId: recipientTask?.id,
				type: explicitByRecipient.has(recipient) ? "teammate_handoff" : "dependency_handoff",
				message,
