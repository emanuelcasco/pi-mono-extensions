...
const DelegateChainParallelStepParams = Type.Object({
	parallel: Type.Array(DelegateTaskParams, { minItems: 1 }),
...
function registerDelegateTools(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		description:
			"Run a foreground delegation group. Use tasks[] for bounded parallel fan-out, or chain[] for sequential / fan-out workflows with {task}, {previous}, and {chain_dir} substitution.",
