...
export class TeamManager {
  ...
  async getTeamSummary(teamId: string): Promise<TeamSummary> {
    const [team, tasks, approvals, teammateProcesses, allSignals] =
      await Promise.all([
      ...
      return {
        ...
        status:
          process?.state === "running"
            ? "in_progress"
            : (activeTask?.status ?? (anyTask ? "idle" : "not_started")),
        currentTask:
          activeTask?.title ??
          tasks.find((t) => t.id === process?.taskId)?.title,
        summary:
          process?.state === "running"
    ...
    const milestonePriority = ["in_progress", "ready", "todo"] as const;
  ...
  async getTeammateSummary(
    ...
    const outputsDir = join(this.store.getTeammateDir(teamId, role), "outputs");
    try {
