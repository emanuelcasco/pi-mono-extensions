...
const DEFAULT_FIXERS: FixerRule[] = [
	{
		...
		command: "npx --no-install eslint --fix --no-error-on-unmatched-pattern {files}",
	},
	{
		label: "black",
		extensions: [".py"],
		command: "black -q {files}",
	},
	{
		label: "prettier",
		extensions: [".json", ".md", ".yml", ".yaml", ".css", ".scss", ".html"],
		command: "npx --no-install prettier --write --log-level=warn {files}",
	},
];
...
function matchFixer(absPath: string, fixers: FixerRule[]): FixerRule | undefined {
...
export default function (pi: ExtensionAPI): void {
	...
	async function flush(paths: string[], ctx: ExtensionContext): Promise<void> {
		...
		// Snapshot mtimes so we can tell which files the fixer actually touched.
		const before = new Map<string, number>();
		for (const bucket of groups.values()) {
			for (const p of bucket) before.set(p, mtimeSafe(p));
		}
		...
		let changed = 0;
		for (const [p, oldMtime] of before) {
			if (mtimeSafe(p) !== oldMtime) {
		...
		if (changed > 0 || failures.length) {
			const parts: string[] = [`auto-fix: ${changed}/${total} file${total === 1 ? "" : "s"} updated`];
			if (failures.length) parts.push(`failed: ${failures.join(", ")}`);
			ctx.ui.notify(`[auto-fix] ${parts.join(" — ")}`, failures.length ? "warning" : "info");
		}
	}
}
